public class leetcode3{
	...
	//（1） Min-avg-slice
 	//给定一个长度为n的整数数组，找到一个连续的子数组，数组元素的平均值最小。 
	 //数据范围N [1..10^5],数组元素范围[-10^4, +10^4]。
	//要求复杂度： 时间O(N),空间O(N)。
	...
	//(2) Passing-cars
	//给定一个长度为N的0-1数组A，它表示一条路上的车流方向,下标从0开始，要找到0<=P<=Q<N，
	//并且A[P] = 0, A[Q] = 1的下标对数。数据范围N [1..10^5]。
	//要求复杂度： 时间O(N),空间O(1)。
	//分析： 实质对于每个0，我们求它后面有多少个1即可。所以我们倒着遍历数组，对每个0，
	//看一下截止到目前为止的后缀和即可。
