...
public class leetcode{
	...
	//Perm-Missing-Elem:
	//一个长度为N的数组里，都是整数，所有的数都不相同，范围是[1..N + 1]，这意味着有一个整数缺失了，
	//找到这个缺失的整数，N的范围[0..10^5], 要求时间复杂度O(N)，空间复杂度O(1)。
	...
	//为P分割的两部分的差值，求最小的差值。 N在[2..10^5]，数组中数据范围[-1000,+1000]。
	//要求复杂度时间O(N)，空间O(N)。
	...
	//Binary-gap:
	//一个整数的2进制表示中，两个1bit直接全部是0, 0的个数叫做一个binary-gap，给定Ｎ，求其最长binary-gap。
	//（两个1之间最大的连续0的个数）。
	...
	//Dominator:
	//找众数，即数组中出现次数超过一半的数。数组长度N [0..10^6] (有0），每个数组元素，
	//整数[-2147483648, +2147483647]。要求时间复杂度O(N)，空间复杂度O(1)。
	...
	//Max-profit:
	//给定数组是股票价钱，要在早时买，晚些时候卖（可以是同一天），求最大收益。天数N [1..10^5],
	//每天的价格[0..10^9]。要求复杂度时间O(N)，空间O(1)。
